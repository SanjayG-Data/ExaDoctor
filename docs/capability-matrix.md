...
| Source | Exists | Columns confirmed | Row access | Notes |
|---|---|---|---|---|
| `EXA_SQL_DAILY` | Yes | `CLUSTER_NAME`, `INTERVAL_START`, `COMMAND_NAME`, `COMMAND_CLASS`, `SUCCESS`, `COUNT`, `DURATION_AVG`/`MAX`, `CPU_AVG`/`MAX`, I/O columns, `NET_AVG`/`MAX`, `ROW_COUNT_AVG`/`MAX`, `EXECUTION_MODE` (one row per day per (`COMMAND_NAME`, `COMMAND_CLASS`, `SUCCESS`, `EXECUTION_MODE`) group, not one row per day) | OK (347 rows spanning 37 distinct days on the probed instance) | Accumulates indefinitely like `EXA_DB_SIZE_DAILY`/`EXA_MONITOR_DAILY` -- the collector windows it to 90 days. Only `COUNT`/`DURATION_AVG` are actually selected. Basis for `SQL-WORKLOAD-TREND-001` -- aggregates `COUNT` and `COUNT × DURATION_AVG` across a day's groups first, since this table (unlike `EXA_MONITOR_DAILY`) isn't already one row per day. |
...
| Multi-day SQL workload volume/execution-time trend | Available | `EXA_SQL_DAILY` (`SQL-WORKLOAD-TREND-001`) |
