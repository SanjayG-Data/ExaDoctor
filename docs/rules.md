...
| `STORAGE-GROWTH-001` | `EXA_DB_SIZE_DAILY` | Compares the latest daily `STORAGE_SIZE_AVG` to the trailing-history median; flags growth beyond a policy-defined ratio AND a policy-defined absolute-GiB floor (a ratio-only check would flag a dev/test instance moving from e.g. 0.2 GiB to 0.35 GiB as a WARNING for a clinically meaningless absolute change). Trend-relative to the instance's own history, not an absolute capacity judgement. |
...
| `SYS-RESOURCE-TREND-001`, `STORAGE-GROWTH-001`, `SYS-RAM-SIZING-001`, `SQL-WORKLOAD-TREND-001` | (see above) | **Single-cluster assumption**: `EXA_MONITOR_DAILY`/`EXA_DB_SIZE_DAILY`/`EXA_SQL_DAILY`/`EXA_SYSTEM_EVENTS` are all documented as per-cluster tables. None of these four rules partition their aggregation by `CLUSTER_NAME` — on a genuine multi-cluster deployment (untested so far; every instance verified against is single-cluster), each degrades to `NOT_EVALUATED` if it detects more than one distinct `CLUSTER_NAME` in its window, rather than silently blending rows from different clusters into one number. Found by independent code review. |
